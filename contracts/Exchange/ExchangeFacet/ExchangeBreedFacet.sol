  ...
  function pregnancyCheckup(Asset memory matron, Asset memory sire) internal {
    ...
    if (pregnancyM.count > 0 || pregnancyS.count > 0) {
      if (
        ...
        (
          pregnancyM.count > 13
            ? ExchangeStorage.layout()._pregnancyMaxTime
            : (ExchangeStorage.layout()._pregnancyTimeLimit * (2 ** pregnancyM.count)).toUint64()
        )
      ) {
      ...
      if (
        ...
        (
          pregnancyS.count > 13
            ? ExchangeStorage.layout()._pregnancyMaxTime
            : (ExchangeStorage.layout()._pregnancyTimeLimit * (2 ** pregnancyS.count)).toUint64()
        )
      ) {
        revert GenesPregnancyTimeLimitExceed();
      }
