...
import { IERC721LootBoxA, BoxConfig } from "./interfaces/IERC721LootBox.sol";
...
abstract contract ERC721LootBoxSimpleA is IERC721LootBoxA, ERC721Simple, TopUp {
  ...
  function unpack(uint256 tokenId) public virtual {
    ...
    // if min == max AND items.length === 1, no need to call random function.
    if (_itemData[tokenId].length == 1 && minMax[0].min == minMax[0].max ) {
  ...
  function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal virtual {
    ...
    for (uint256 i = 0; i < itemsLength; i++) {
      ...
      uint256 actualCount = (randomValue % (boxConfig[i].max - boxConfig[i].min + 1)) + boxConfig[i].min;
