...
export function shouldUnpackBox(factory: () => Promise<any>) {
  ...
  const erc998Factory = (name: string) => deployERC998(name);
  ...
  describe("Unpack box", function () {
    ...
    describe("MIX", function () {
      it("should mint/unpack multiple", async function () {
        ...
        const tx1 = lootboxInstance.mintBox(
          ...
          [{ min: 5, max: 5 },{ min: 5, max: 5 },{ min: 5, max: 5 },{ min: 5, max: 5 },{ min: 5, max: 5 }],
        );
    ...
    describe("MIX RANDOM", function () {
      it("should mint:1, unpack random: 1-5", async function () {
        ...
        console.log("add", await erc721SimpleInstance.getAddress());
        ...
        // BOX with 2 common 721 and 1 1155 with random amount 0 - 5
        const tx1 = lootboxInstance.mintBox(
          receiver.address,
          templateId,
          [
        ...
        if (network.name === "hardhat") {
         await randomFixRequest(lootboxInstance, vrfInstance,101); // rnd101 - will get items[2].amount = 3;
        }


        const balance = await erc721SimpleInstance.balanceOf(receiver.address);
        console.log("balance721", balance)
        expect(balance).to.equal(138);

        const balance2 = await erc1155SimpleInstance.balanceOf(receiver.address, 1);
        console.log("balance1155", balance2)
