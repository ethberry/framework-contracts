...
export function shouldBetPositionNative(factory: () => Promise<any>, isVerbose = false) {
  describe("betPositionNative", function () {
    it("should allow betting with native Ether", async function () {
      const { prediction, bettor1, bettor2, betAsset, startTimestamp, initialBalance1, initialBalance2 } =
        await factory();
      ...
      await expect(tx1)
        ...
        .withArgs(
          1,
          bettor1.address,
          [betAsset.tokenType, betAsset.token, betAsset.tokenId, betAmount1],
          Position.LEFT,
        );
      ...
      await expect(tx2)
        ...
        .withArgs(
          1,
          bettor2.address,
          [betAsset.tokenType, betAsset.token, betAsset.tokenId, betAmount2],
          Position.RIGHT,
        );
