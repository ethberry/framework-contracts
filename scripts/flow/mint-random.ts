...
async function main() {
  ...
  const tx5 = await vrfInstance.addConsumer(subId, address);
  await tx5.wait();
  ...
  const tx6 = await contractInstance.setSubscriptionId(subId);
  await tx6.wait();

  const eventFilter4 = contractInstance.filters.VrfSubscriptionSet();
  const events4 = await contractInstance.queryFilter(eventFilter4);
  const result4 = recursivelyDecodeResult(events4[0].args as unknown as Result);
  console.info("VrfSubscriptionSet", result4);

  const tx7 = await contractInstance.mintRandom(owner, 1);
  await tx7.wait();
  ...
  const events6 = await contractInstance.queryFilter(eventFilter6, blockNum);
